...
public class ViewCryoLinkStation extends ViewData  {
    ...
    public ViewCryoLinkStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetCryoLinkStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.CONTROL_POINT.button(e -> {
           CreateAndShowCryoLinkActionsSidePopupView("CRYOLINKACTIONS", name + "Actions");
           setMouseTransparent(false);
        }));
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowCryoLinkFlagsSidePopupView("CRYOLINKSTATIONFLAGS", name);
           setMouseTransparent(false);
        }));
    ...
    void CreateAndShowCryoLinkActionsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewCryoLinkActions(fxml, name.replace("Actions",""));
    ...
    void CreateAndShowCryoLinkFlagsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewCryoLinkStation(fxml, name);
          System.out.println("fxml = " + fxml + " name = " + name);
          Label viewName = (Label) content.pane.lookup("#ViewName");
