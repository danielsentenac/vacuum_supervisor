...
public class ViewLinkStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewLinkStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetLinkStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.CONTROL_POINT.button(e -> {
           CreateAndShowLinkActionsSidePopupView("LINKACTIONS", name + "Actions");
           setMouseTransparent(false);
        }));
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowLinkFlagsSidePopupView("LINKSTATIONFLAGS", name);
           setMouseTransparent(false);
        }));
    ...
    void CreateAndShowLinkActionsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewLinkActions(fxml, name.replace("Actions",""));
    ...
    void CreateAndShowLinkFlagsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewLinkStation(fxml, name);
          Label viewName = (Label) content.pane.lookup("#ViewName");
