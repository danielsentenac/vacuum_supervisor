...
public class ViewTowerBenchStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewTowerBenchStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetTowerBenchStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowSidePopupView("TOWERBENCHSTATIONFLAGS", name);
    ...
    void CreateAndShowSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTowerBenchStation(fxml, name);
