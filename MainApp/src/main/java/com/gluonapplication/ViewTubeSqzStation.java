...
public class ViewTubeSqzStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewTubeSqzStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetTubeSqzStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.CONTROL_POINT.button(e -> {
           CreateAndShowTubeSqzActionsSidePopupView("TUBESQZACTIONS", name + "Actions");
           setMouseTransparent(false);
        }));
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowTubeSqzSidePopupView("TUBESQZSTATIONFLAGS", name);
           setMouseTransparent(false);
        }));
    ...
    void CreateAndShowTubeSqzActionsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTubeSqzActions(fxml, name.replace("Actions",""));
    ...
    void CreateAndShowTubeSqzSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTubeSqzStation(fxml, name);
          Label viewName = (Label) content.pane.lookup("#ViewName");
