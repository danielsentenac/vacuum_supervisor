...
public class ViewCryoStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewCryoStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetCryoStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.CONTROL_POINT.button(e -> {
           CreateAndShowCryoActionsSidePopupView("CRYOACTIONS", name + "Actions");
           setMouseTransparent(false);
        }));
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowCryoFlagsSidePopupView("CRYOSTATIONFLAGS", name);
           setMouseTransparent(false);
        }));
    ...
    void CreateAndShowCryoActionsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewCryoActions(fxml, name.replace("Actions",""));
    ...
    void CreateAndShowCryoFlagsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewCryoStation(fxml, name);
          Label viewName = (Label) content.pane.lookup("#ViewName");
