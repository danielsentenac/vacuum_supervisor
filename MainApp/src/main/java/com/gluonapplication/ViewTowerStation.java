...
public class ViewTowerStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewTowerStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetTowerStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowSidePopupView("TOWERSTATIONFLAGS", name);
    ...
    void CreateAndShowSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTowerStation(fxml, name);
