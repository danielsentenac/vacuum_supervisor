...
public class ViewTube extends ViewData  {

    private String fxml;
    private String name;

    public ViewTube(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetTube(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           if (name.equals("TUBEN"))
              CreateAndShowTubeFlagsSidePopupView("TUBENFLAGS", name);
           else if (name.equals("TUBEW"))
              CreateAndShowTubeFlagsSidePopupView("TUBEWFLAGS", name);
    ...
    void CreateAndShowTubeFlagsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTube(fxml, name);
