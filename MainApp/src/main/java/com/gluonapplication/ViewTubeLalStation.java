...
public class ViewTubeLalStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewTubeLalStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetTubeLalStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowSidePopupView("TUBELALSTATIONFLAGS", name);
    ...
    void CreateAndShowSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTubeLalStation(fxml, name);
