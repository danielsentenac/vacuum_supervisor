...
public class ViewCentralBuilding extends ViewData  {

    private String fxml;
    private String name;

    public ViewCentralBuilding(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetCentralBuilding(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowSidePopupView("CENTRALBUILDINGFLAGS", name);
    ...
    void CreateAndShowSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewCentralBuilding(fxml, name);
