...
public class ViewTubeMCStation extends ViewData  {

    private String fxml;
    private String name;

    public ViewTubeMCStation(String _fxml, String _name) {
        super(_fxml, _name);
        fxml = _fxml;
        name = _name;
        data = new DataSetTubeMCStation(name);
    }
    ...
    protected void updateAppBar(AppBar appBar) {
        ...
        appBar.getActionItems().addAll(MaterialDesignIcon.CONTROL_POINT.button(e -> {
           CreateAndShowTubeMCActionsSidePopupView("TUBEMCACTIONS", name + "Actions");
           setMouseTransparent(false);
        }));
        appBar.getActionItems().addAll(MaterialDesignIcon.FLAG.button(e -> {
           CreateAndShowTubeMCFlagsSidePopupView("TUBEMCSTATIONFLAGS", name);
           setMouseTransparent(false);
        }));
    ...
    void CreateAndShowTubeMCActionsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTubeMCActions(fxml, name.replace("Actions",""));
    ...
    void CreateAndShowTubeMCFlagsSidePopupView(String fxml, String name) {
       if ( !MobileApplication.getInstance().isLayerPresent(name) ) {
          SidePopupViewData content = new SidePopupViewTubeMCStation(fxml, name);
          Label viewName = (Label) content.pane.lookup("#ViewName");
