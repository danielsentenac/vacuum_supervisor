...
!define APP_EXE "vacuum-supervisor.vbs"
...
Icon "${STAGE_DIR}/vacuum-supervisor-shortcut.ico"
UninstallIcon "${STAGE_DIR}/vacuum-supervisor-shortcut.ico"
...
Section "Install"
  ...
  File "${STAGE_DIR}/vacuum-supervisor.vbs"
  File "${STAGE_DIR}/vacuum-supervisor.png"
  File "${STAGE_DIR}/vacuum-supervisor-shortcut.ico"

  CreateDirectory "${START_MENU_DIR}"
  CreateShortcut "${START_MENU_DIR}\Vacuum Supervisor.lnk" "$SYSDIR\wscript.exe" '"$INSTDIR\${APP_EXE}"' "$INSTDIR\vacuum-supervisor-shortcut.ico"
  CreateShortcut "${START_MENU_DIR}\Uninstall Vacuum Supervisor.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\vacuum-supervisor-shortcut.ico"
  CreateShortcut "$DESKTOP\Vacuum Supervisor.lnk" "$SYSDIR\wscript.exe" '"$INSTDIR\${APP_EXE}"' "$INSTDIR\vacuum-supervisor-shortcut.ico"
  ...
  WriteRegStr HKCU "Software\Classes\${APP_PROTOCOL}\DefaultIcon" "" "$INSTDIR\vacuum-supervisor-shortcut.ico,0"
  WriteRegStr HKCU "Software\Classes\${APP_PROTOCOL}\shell\open\command" "" '"$SYSDIR\wscript.exe" "$INSTDIR\${APP_EXE}" "%1"'
  ...
  WriteRegStr HKCU "${UNINSTALL_REG_KEY}" "DisplayIcon" "$INSTDIR\vacuum-supervisor-shortcut.ico"
